...
//        alert.view = UIImageView(frame: CGRect(x: 12, y: 12, width: 12, height: 12))
   ...
   @objc func buttonClicked() {
    ...
    let height:NSLayoutConstraint = NSLayoutConstraint(item: alert.view, attribute: NSLayoutConstraint.Attribute.height, relatedBy: NSLayoutConstraint.Relation.equal, toItem: nil, attribute: NSLayoutConstraint.Attribute.notAnAttribute, multiplier: 1, constant: self.view.frame.height * 0.15)
      alert.view.addConstraint(height);
    alert.addAction(UIAlertAction(title: "Next", style: .default, handler: {
        (action :UIAlertAction!) -> Void in self.nextBtnClick()
